...
class StudentAuthController(
    ...
    @PostMapping("/login")
    fun login(@Valid @RequestBody request: StudentLoginRequest): ResponseEntity<StudentLoginResponse> {
        val response = studentAuthService.login(request)
        return ResponseEntity.ok(response)
    }

    @PostMapping("/refresh")
    fun refresh(@Valid @RequestBody request: StudentRefreshTokenRequest): ResponseEntity<StudentRefreshTokenResponse> {
