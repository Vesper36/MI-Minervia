...
class StudentApiClient {
  ...
  constructor() {
    if (typeof window !== 'undefined') {
      this.accessToken = localStorage.getItem('studentAccessToken');
      this.refreshToken = localStorage.getItem('studentRefreshToken');
    }
  }

  setAuthChangeCallback(callback: AuthChangeCallback | null) {
  ...
  private async tryRefreshToken(): Promise<boolean> {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/student/auth/refresh`, {
        method: 'POST',
